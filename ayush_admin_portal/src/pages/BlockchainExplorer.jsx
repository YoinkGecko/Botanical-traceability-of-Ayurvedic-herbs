...
import { ArrowLeft, Search, Database, Filter, User, Package, Beaker, Factory } from 'lucide-react';
...
const BlockchainExplorer = () => {
  ...
  const stats = [
    ...
    { label: 'Verified', value: transactions?.filter(t => t?.status === 'verified')?.length, color: 'text-green-600' },
    { label: 'Pending', value: transactions?.filter(t => t?.status === 'pending')?.length, color: 'text-orange-600' },
    { label: 'Today', value: transactions?.filter(t => t?.timestamp?.includes('2024-01-15'))?.length, color: 'text-purple-600' }
  ];
          ...
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                ...
                <h1 className="text-xl font-bold text-gray-900">Blockchain Explorer</h1>
              </div>
            </div>
          </div>
        </div>
                  ...
                  return (
                    <tr key={transaction?.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-xs text-gray-500 font-mono">{transaction?.hash}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <TypeIcon size={16} className="mr-2 text-gray-400" />
